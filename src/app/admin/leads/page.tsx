      ...
      {isLoading ? (
        ...
        <div className="overflow-hidden rounded-[var(--radius-md)] border border-[var(--allone-line)] bg-[var(--bg-surface)] shadow-[var(--shadow-xs)]">
          {leads.map((lead, _idx) => (
            <div
              key={lead.id}
              className={`group p-4 transition-colors hover:bg-[var(--bg-surface-alt)] ${_idx > 0 ? "border-t border-[var(--allone-line-soft)]" : ""}`}
            >
              <div className="flex items-start justify-between gap-3">
                {/* Lead info */}
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    ...
                    {lead.source_url &&
                      !INFOSHOP_PATTERN.test(lead.source_url) && (
                        <a
                  ...
                  <div className="flex items-center gap-2 mt-2 text-xs text-[var(--ink-400)]">
                    ...
                    {lead.matched_service && (
                      <span>· {lead.matched_service}</span>
                    )}
                    <span>· {formatDate(lead.created_at)}</span>
                  </div>
                  {/* Pitch reasons */}
                  {lead.tags &&
                    lead.tags.filter((t: string) => !HIDDEN_TAGS.has(t))
                      .length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {lead.tags
                          .filter((t: string) => !HIDDEN_TAGS.has(t))
                          .map((tag: string) => (
                            <span
                              key={tag}
