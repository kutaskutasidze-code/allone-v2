...
import { Search, X, Users, MessageSquare, ExternalLink, Phone, Mail, Globe, Trash2, BarChart3, CalendarClock } from 'lucide-react';
import { EmptyState, LeadStatusDropdown } from '@/components/admin';
...
function CallbackBadge({ callbackDate }: { callbackDate: string }) {
  const cb = formatCallbackLabel(callbackDate);
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {(lead as { callback_date?: string | null }).callback_date && lead.status === 'callback' && (
                    <CallbackBadge callbackDate={(lead as { callback_date: string }).callback_date} />
                  )}
                  <LeadStatusDropdown
                    leadId={lead.id}
                    currentStatus={lead.status}
                    currentCallbackDate={(lead as { callback_date?: string | null }).callback_date}
