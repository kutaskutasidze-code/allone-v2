...
function StatusPill({ status }: { status: string | null }) {
  if (!status) return <span className="text-xs text-[var(--ink-400)] italic">created</span>;
  const cls = LEAD_STATUS_STYLES[status] || 'bg-[var(--bg-sunken)] text-[var(--ink-700)]';
  return (
...
export default function AdminAuditPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold tracking-tight text-[var(--ink-900)] font-display">Audit Log</h1>
        <p className="mt-1.5 text-sm text-[var(--ink-500)]">Every lead status change. {total} entries match current filters.</p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="text-xs text-[var(--ink-500)] hover:text-[var(--ink-900)] underline">
              Clear filters
            </button>
          )}
        </div>
        ...
        <table className="w-full min-w-[700px] text-sm">
          ...
          <tbody>
            {loading && rows.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-xs text-[var(--ink-400)]">Loading…</td></tr>
            ) : rows.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-10 text-center">
                  <ClipboardList className="w-6 h-6 mx-auto text-[var(--ink-300)] mb-2" />
                  <p className="text-sm text-[var(--ink-500)]">No audit entries match your filters.</p>
                </td>
              </tr>
            ) : (
              rows.map(row => (
                <tr key={row.id} className="border-t border-[var(--allone-line-soft)] hover:bg-[var(--bg-surface-alt)]/50 transition-colors">
                  <td className="px-4 py-3 text-[var(--ink-700)] whitespace-nowrap" title={formatAbsolute(row.changedAt)}>
                    <div className="text-xs">{formatRelative(row.changedAt)}</div>
                    <div className="text-[11px] text-[var(--ink-400)]">{formatAbsolute(row.changedAt)}</div>
                  </td>
                  <td className="px-4 py-3">
                    {row.lead ? (
                      <Link href={`/admin/leads?search=${encodeURIComponent(row.lead.name)}`} className="block group">
                        <div className="font-medium text-[var(--ink-900)] group-hover:underline truncate max-w-[260px]">{row.lead.company || row.lead.name}</div>
                        {row.lead.company && <div className="text-xs text-[var(--ink-500)] truncate max-w-[260px]">{row.lead.name}</div>}
                      </Link>
                    ) : (
                      <span className="text-xs text-[var(--ink-400)] italic">deleted lead</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-[var(--ink-700)]">
                    {row.salesUser?.name || <span className="text-xs text-[var(--ink-400)]">unassigned</span>}
                  </td>
                  <td className="px-4 py-3">
                    <div className="inline-flex items-center gap-2">
                      <StatusPill status={row.fromStatus} />
                      <ArrowRight className="w-3 h-3 text-[var(--ink-300)]" />
