...
function ApplicationsContent() {
  ...
  const setStatus = async (id: string, status: string) => {
    setApps((prev) => prev.map((a) => (a.id === id ? { ...a, status: status as ApplicationStatus } : a)));
  ...
  return (
    <div>
      <Link href="/admin/careers" className="inline-flex items-center gap-1.5 text-sm text-[var(--ink-500)] hover:text-[var(--ink-900)] mb-4">
        <ArrowLeft className="h-4 w-4" />
        Vacancies
      </Link>
      <PageHeader title="Applications" description="Review and triage applicants. CV links are signed and expire in 60 seconds." />

      <div className="flex flex-wrap items-center gap-2 mb-4">
        <select className={inputCls} value={vacancyFilter} onChange={(e) => setVacancyFilter(e.target.value)}>
        ...
        <span className="ml-auto text-sm text-[var(--ink-500)]">{apps.length} application{apps.length === 1 ? '' : 's'}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {apps.map((a) => (
            <div key={a.id} className="bg-[var(--bg-surface)] border border-[var(--allone-line)] rounded-[var(--radius-sm)] px-4 py-3">
              <div className="flex items-start gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-medium text-[var(--ink-900)]">{a.name}</span>
                    <span className="text-xs text-[var(--ink-400)]">→ {a.vacancy_title || 'Unlinked'}</span>
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-[var(--ink-500)]">
                    <a href={`mailto:${a.email}`} className="inline-flex items-center gap-1 hover:text-[var(--ink-900)]">
                  ...
                  {a.projects && (
                    <p className="mt-2 text-sm text-[var(--ink-700)] whitespace-pre-wrap break-words"><span className="text-[var(--ink-400)]">Projects: </span>{a.projects}</p>
                  )}
                  {a.note && (
                    <p className="mt-1 text-sm text-[var(--ink-700)] whitespace-pre-wrap break-words"><span className="text-[var(--ink-400)]">Note: </span>{a.note}</p>
                  )}
                </div>
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <span className={`inline-flex px-2 py-0.5 text-[11px] font-medium rounded-[var(--radius-xs)] ring-1 ${statusStyle[a.status]}`}>
                    {APPLICATION_STATUSES.find((s) => s.value === a.status)?.label || a.status}
                  </span>
                  <select className={`${inputCls} text-xs py-1`} value={a.status} onChange={(e) => setStatus(a.id, e.target.value)}>
                    {APPLICATION_STATUSES.map((s) => (
                      <option key={s.value} value={s.value}>{s.label}</option>
                    ))}
                  </select>
