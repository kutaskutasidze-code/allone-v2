...
function DistributionBar({ data, names, colors }: { data: Record<string, number>; names: Record<string, string>; colors?: Record<string, string> }) {
  ...
  if (total === 0) return <p className="text-sm text-[var(--ink-400)]">No data</p>;
        ...
        return (
          <div key={key}>
            <div className="flex items-center justify-between text-xs mb-1">
              ...
              <span className="text-[var(--ink-400)]">{value} ({pct}%)</span>
            </div>
        ...
        {statCards.map(card => {
          ...
          return (
            <div key={card.label} className="p-5 bg-[var(--bg-surface)] border border-[var(--allone-line-soft)] rounded-[var(--radius-md)] shadow-[var(--shadow-xs)] shadow-black/[0.02]">
              ...
              <p className="text-2xl font-semibold text-[var(--ink-900)]">{card.value}</p>
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-[var(--ink-900)] font-medium">{data.overview.phoneRate}%</span>
              </div>
              <div className="h-1.5 bg-[var(--bg-sunken)] rounded-full overflow-hidden">
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-[var(--ink-900)] font-medium">{data.overview.emailRate}%</span>
              </div>
              <div className="h-1.5 bg-[var(--bg-sunken)] rounded-full overflow-hidden">
                <div className="h-full bg-[var(--ao-accent)] rounded-full" style={{ width: `${data.overview.emailRate}%` }} />
              </div>
            </div>
          </div>
        </div>
