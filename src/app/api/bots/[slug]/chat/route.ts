...
function buildSystem(
  ...
  return [
    ...
    goals,
    ``,
    `როცა საკმარისი ინფორმაცია გექნება ყველა (ან თითქმის ყველა) თემაზე: დაწერე მოკლე დამამთავრებელი წინადადება (მადლობა + რომ მალე გადასცემ შეთავაზებას), შემდეგ ახალ ხაზზე ზუსტად "<<COMPLETE>>", შემდეგ ახალ ხაზზე JSON ობიექტი, სადაც გასაღები არის კითხვის id და მნიშვნელობა — მოპოვებული პასუხი. სხვა არაფერი JSON-ის შემდეგ.`,
  ]
    .filter(Boolean)
    .join("\n");
}
