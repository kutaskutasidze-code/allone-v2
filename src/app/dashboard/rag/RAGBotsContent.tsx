  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[22px] font-semibold text-[#1d1d1f] tracking-tight">RAG Bots</h1>
          <p className="text-[13px] text-[#86868b] mt-0.5">{projects.length} of {limit} bots</p>
        </div>
        {canCreate && (
          <Link
      ...
      {projects.length > 0 ? (
        ...
        >
          <div className="w-12 h-12 rounded-2xl bg-[#f5f5f7] flex items-center justify-center mx-auto mb-4">
            <MessageSquare className="w-5 h-5 text-[#86868b]" />
          </div>
          <h3 className="text-[15px] font-medium text-[#1d1d1f] mb-1">No bots yet</h3>
          <p className="text-[13px] text-[#86868b] mb-5">Create chatbots that answer from your documents.</p>
          <Link
            href="/dashboard/studio"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-[#1d1d1f] text-white text-[13px] font-medium rounded-full hover:bg-[#3a3a3c] transition-colors"
          >
