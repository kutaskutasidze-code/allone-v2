...
export function SalesSidebar({ isCollapsed, onToggle, isMobileOpen, onMobileClose }: SalesSidebarProps) {
  ...
  const desktopSidebarContent = (
    <div className="flex h-full flex-col relative">
      ...
      <div className="border-t border-[var(--allone-line-soft)] dark:border-slate-800 p-3 space-y-0.5">
        ...
        <div className="relative group">
          <Link href="/" target="_blank" className={cn('flex items-center rounded-[var(--radius-sm)] text-[13px] font-medium text-[var(--ink-500)] hover:text-[var(--ink-900)] hover:bg-[var(--bg-surface-alt)] dark:text-[var(--ink-400)] dark:hover:text-slate-100 dark:hover:bg-slate-800 transition-colors duration-150', isCollapsed ? 'justify-center px-2 py-2.5' : 'gap-3 px-3 py-2')}>
        ...
        <div className="relative group">
          <button onClick={handleLogout} className={cn('flex w-full items-center rounded-[var(--radius-sm)] text-[13px] font-medium text-[var(--ink-500)] hover:text-[var(--ink-900)] hover:bg-[var(--bg-surface-alt)] dark:text-[var(--ink-400)] dark:hover:text-slate-100 dark:hover:bg-slate-800 transition-colors duration-150', isCollapsed ? 'justify-center px-2 py-2.5' : 'gap-3 px-3 py-2')}>
      ...
      <button onClick={onToggle} className="absolute -right-3 top-20 w-6 h-6 bg-[var(--bg-surface)] border border-[var(--allone-line)] rounded-full flex items-center justify-center text-[var(--ink-400)] hover:text-[var(--ink-900)] hover:border-[var(--allone-line-strong)] transition-all duration-150 shadow-[var(--shadow-xs)] z-50">
  ...
  const mobileSidebarContent = (
    <div className="flex h-full flex-col">
      <div className="flex h-14 items-center justify-between border-b border-[var(--allone-line-soft)] dark:border-slate-800 px-5">
        ...
        <button onClick={onMobileClose} className="w-8 h-8 flex items-center justify-center rounded-[var(--radius-sm)] text-[var(--ink-400)] hover:text-[var(--ink-900)] hover:bg-[var(--bg-surface-alt)] transition-colors">
          <X className="h-5 w-5" />
        </button>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-4">
      ...
      <div className="border-t border-[var(--allone-line-soft)] dark:border-slate-800 p-3 space-y-0.5">
        <button onClick={toggleTheme} className="flex w-full items-center gap-3 rounded-[var(--radius-sm)] px-3 py-2.5 text-[13px] font-medium text-[var(--ink-500)] hover:text-[var(--ink-900)] hover:bg-[var(--bg-surface-alt)] dark:text-[var(--ink-400)] dark:hover:text-slate-100 dark:hover:bg-slate-800 transition-colors duration-150">
          {theme === 'dark' ? <Sun className="h-[18px] w-[18px]" /> : <Moon className="h-[18px] w-[18px]" />}
          <span>{theme === 'dark' ? 'Light mode' : 'Dark mode'}</span>
        </button>
        <Link href="/" target="_blank" onClick={handleNavClick} className="flex items-center gap-3 rounded-[var(--radius-sm)] px-3 py-2.5 text-[13px] font-medium text-[var(--ink-500)] hover:text-[var(--ink-900)] hover:bg-[var(--bg-surface-alt)] dark:text-[var(--ink-400)] dark:hover:text-slate-100 dark:hover:bg-slate-800 transition-colors duration-150">
          <ExternalLink className="h-[18px] w-[18px]" />
          <span>View Website</span>
        </Link>
        <button onClick={handleLogout} className="flex w-full items-center gap-3 rounded-[var(--radius-sm)] px-3 py-2.5 text-[13px] font-medium text-[var(--ink-500)] hover:text-[var(--ink-900)] hover:bg-[var(--bg-surface-alt)] dark:text-[var(--ink-400)] dark:hover:text-slate-100 dark:hover:bg-slate-800 transition-colors duration-150">
  ...
  return (
    <>
      ...
      <button onClick={onToggle} className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-[var(--bg-surface)] border border-[var(--allone-line)] rounded-[var(--radius-sm)] flex items-center justify-center text-[var(--ink-700)] hover:text-[var(--ink-900)] shadow-[var(--shadow-xs)] transition-colors" aria-label="Open menu">
